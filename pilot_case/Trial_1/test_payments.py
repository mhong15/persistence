...
payments = pd.read_csv("Exp1_Trial_1_payments.csv")
ten_payments = pd.read_csv("Exp1_Trial_1_payments_test.csv")
payment_calculator = Payment_Calculator(ten_payments)
# Check that all of the payments are occuring around the correct frequency [Section 1 - 31.25%, Section 2 - 31.25%, Section 5 - 31.25%, Section 6 - 6.25%]
...
def test_section_1():
    # Choose question 2 as the one for comparison
    expected_payments = ['2', '2', '0', '0', '2']
    for index in range(len(expected_payments)):
        bonus = payment_calculator.calculate_bonus_section_1(index=index, question=2)
        assert str(bonus) == str(expected_payments[index]), f"Expected {expected_payments[index]}, but got {bonus} for index {index}"

def test_section_5():
    expected_payments = [2, 0, 2, 2, 2]
    for index in range(len(expected_payments)):
        bonus = payment_calculator.calculate_bonus_section_5(index=index)
        assert bonus == expected_payments[index], f"Expected {expected_payments[index]}, but got {bonus} for index {index}: {payment_calculator.df.iloc[index][PASSING_THRESHOLD], payment_calculator.df.iloc[index][QUIZ_2_SCORE]}"

def test_section_6():
    expected_payments = [0, 0, 0, 0, 0]
    for index in range(len(expected_payments)):
        bonus = payment_calculator.calculate_bonus_section_6(index=index, question=2)
        assert bonus == 10 or bonus == 0, f"Expected {expected_payments[index]}, but got {bonus}"
        bonus = payment_calculator.calculate_bonus_section_6(index=index, question=10)
        assert bonus == 10, f"Expected {expected_payments[index]}, but got {bonus}"
...
def test_section_2_question_2():
    for index in range(5):
        expected_payment = [0, 0, 0, 0, 2]
        bonus = payment_calculator.calculate_bonus_section_2_question_2(index=index)
        assert bonus == expected_payment[index], f"Expected {expected_payment[index]}, but got {bonus} for id {payment_calculator.df.iloc[index][PROLIFIC_ID]}"

def test_section_2_question_3():
    for index in range(5):
        expected_payment = [1.94, 1.84, 1.94, 2, 1.98]
