...
class Payment_Calculator:
    ...
    def calculate_all_payments(self):
        ...
        for row in range(self.df.index.size):
            ...
            self.payment_df = self.payment_df._append({'prolific_id': prolific_id, 'bonus_payment_section': section, 'payment': bonus }, ignore_index=True)
    ...
    def calculate_bonus_section_2_question_1(self, index):
        ...
        p = random.randint(1, 100) # Generating p
    ...
    def calculate_bonus_section_2_question_3(self, index):
        ...
        if pd.isna(g):
            return 0
        print(f"max(100 - abs({c} - {g}), 0) / 100) * 2 = {(max(100 - abs(c - g), 0) / 100) * 2}")
