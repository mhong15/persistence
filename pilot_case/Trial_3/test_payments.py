...
payments = pd.read_csv("Exp1_Trial_3_payments.csv")
data = pd.read_csv("Exp1_Trial_3_cleaned_data.csv")
# Save the first 10 participants of Exp1_Trial_3 to a new file to test the payment calculator
data.head(5).to_csv("Exp1_Trial_3_payments_test.csv", index=False)
ten_payments = pd.read_csv("Exp1_Trial_3_payments_test.csv")
...
def test_section_5():
    expected_payments = [2, 2, 1.8, 0, 2]
    for index in range(1, 5):
        bonus = payment_calculator.calculate_bonus_section_5(index=index)
        assert bonus == expected_payments[index], f"Expected {expected_payments[index]}, but got {bonus}"
